-- ─────────────────────────────────────────────────────────────────────────────
-- Roadmap interactivo DPROMA × 2894 — siembra
--
-- Se ejecuta una vez, después de schema.sql. Es idempotente: los identificadores
-- son fijos (uuid v5 derivado del título), así que volver a ejecutarlo no
-- duplica nada.
--
-- Las 22 iniciativas salen del roadmap de /plan-transformacion. El plan las lista
-- 24 veces porque 2 son continuas y aparecen en dos horizontes; aquí van una
-- sola vez, con una barra larga que los cruza.
--
-- Las fechas de corto plazo son una PROPUESTA de partida para arrastrar en la
-- herramienta, no un compromiso: el plan no fijaba fechas por iniciativa.
-- Medio y largo entran sin fecha y viven en la bandeja lateral.
-- ─────────────────────────────────────────────────────────────────────────────
