...
| 2.4.0 | 2026-09-17 | Rehace la fila de cifras del tablero, que la revisión de QA había despachado con una nota al pie y resultó ser un componente que faltaba (§12.4.1). `.kpi` estaba pensado para **una** cifra por widget, con el rótulo en la cabecera del widget; el tablero necesitaba siete conviviendo en un bloque y lo resolvió por su cuenta. Medido sobre la pantalla: las siete tarjetas miden lo mismo —175×97px— así que no era un problema de tamaño; la cifra está clavada a la misma altura en las siete y el rótulo cae a tres alturas distintas, con 34px de horquilla, porque el interior va anclado abajo y el rótulo sube o baja según cuántas líneas traiga el apoyo. Aparecen otros dos defectos al reproducirlo: la cifra tiene 14px de altura de mayúscula, unos 20 de cuerpo cuando `--fs-kpi` son 30, así que el número va a 1,4 veces su rótulo en vez de a 2,1 y no manda en su tarjeta; y cada cifra va en una caja con relleno y borde dentro de un widget, que es anidar tarjetas, prohibido desde §6.3. Entra `.cifras`: las tres ranuras —cifra, rótulo y apoyo— pasan a ser filas de una misma rejilla compartida por toda la fila con `subgrid`, que es lo que alinea el rótulo tenga o no tenga apoyo; igualar la altura de las cajas no bastaba, porque ya eran todas de 97px. Con ello, tres reglas de composición: el apoyo es de una línea y una definición de la medida baja a la nota de método; una fila de cifras es un grupo con nombre, y las siete del tablero eran cuatro de órdenes y tres de trámites con dos «totales» contiguos —siete columnas no caben en una rejilla de doce, y ése fue el motivo de salirse de `.dash`, pero cuatro de `.w-3` y tres de `.w-4` sí caben—; y un grupo entero en cero se pinta como vacío, que es lo que ese mismo tablero ya hace más abajo con «Nada en tu alcance». El QA previo al merge tumbó una de las reglas tal como estaba escrita: el apoyo se imponía con `text-overflow:ellipsis`, y al ancho mínimo que el propio componente declara el apoyo del ejemplo documentado se cortaba sin forma de leerlo. Se retira el recorte: ser de una línea pasa a ser regla de redacción, con el presupuesto medido —118px, unos 18 caracteres—, y el día que no se cumpla la rejilla absorbe la segunda línea para todas por igual. Queda anotado también que una cifra sola no es una fila: `auto-fit` la estira a todo el widget. El QA destapó además que `auto-fit` deja suelta la última cifra cuando el grupo tiene cinco o seis —seis se reparten 6/1440px, 5+1 a 1024 y 4+2 a 768—, así que queda escrito el techo de cuatro por grupo, que §12.6 ya imponía a la tarjeta de entidad: cinco y seis no tienen ancho en la rejilla de doce, y seis conteos de un mismo total son una barra apilada con leyenda, no seis cajas. Y dos defectos de la propia página que salieron al revisarla: las cuatro figuras de gráfico arrastraban los 40px de margen por defecto de `<figure>`, así que salían 80px más estrechas que su widget y centradas dentro de él; y `.sr` estaba acotada a un componente, de modo que los cuatro `<caption>` escritos para lector de pantalla se pintaban encima de su tabla. Además, el sparkline del ejemplo decía «Cerrados este mes: 20» y traía ocho semanas que sumaban 35, así que §12.5 gana la regla de que la serie y la cifra del mismo widget miden lo mismo y cuadran. Y §10 pasa de seis trampas a nueve con las tres que salieron revisando esta página: el margen por defecto de `<figure>` que nadie reponía y que estrechaba las cuatro gráficas 80px, una utilidad declarada como estilo de un componente —`.sr` acotada a `.filtros-demo`, con sus cuatro `caption` pintándose— y `display:block` sobre un `<summary>`, que le quita la flecha. Las tres comparten forma: no dan error, no desbordan, no rompen nada que se vea, y por eso sobrevivieron a varias revisiones. Y el lienzo de los gráficos reserva la mitad de la marca más gruesa: sin ello la línea de 2px del sparkline se trazaba a caballo del borde y se veía cortada por arriba, medido en cero píxeles de aire contra los tres que deja ahora. Y una corrección estructural que salió al revisar el texto: con tres ranuras, un rótulo que parte en dos renglones hacía crecer esa fila para todas las tarjetas, así que en las que no partían el gris quedaba a **30px de su rótulo** en vez de a los 8 de diseño, y la fila se veía desordenada. El apoyo deja de ser ranura y pasa a ir dentro del bloque `.etq`, pegado a su rótulo pase lo que pase: quedan dos ranuras, la cifra y ese bloque. Se pierde que los grises de tarjetas distintas queden a la misma altura, y está bien perderlo, porque el gris pertenece a su rótulo y no al gris de al lado; se conservan el rótulo alineado y las tarjetas del mismo alto, que era lo que fallaba en QA. El bloque lleva además `overflow-wrap:anywhere`: una referencia sin espacios ni guiones se salía de la caja, medido en 283px de contenido dentro de 170. El rótulo y su apoyo se cierran además a 5px de texto a texto, la mitad de los 10 que había: sin `gap` y con el interlineado del apoyo ceñido a 1,4. El interlineado se toca en el apoyo y no en el rótulo porque ceñir el rótulo acorta también por arriba, y el aire de 4px entre la cifra y el rótulo es el que debe quedarse. Y una prueba de entorno destapó un fallo de la decisión de no llevar borde: en alto contraste del sistema, `forced-colors` lleva todos los fondos a Canvas, así que tarjeta y widget quedaban del mismo blanco y las cajas desaparecían. Se declara `border:1px solid transparent` siempre —para que la geometría no cambie entre modos— y `border-color:CanvasText` bajo `@media (forced-colors: active)`. |
